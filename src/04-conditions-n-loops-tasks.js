...
function findFirstSingleChar(str) {
  ...
  return strArr.find((char) => charCount[char] === 1) || null;
}
...
function isBracketsBalanced(str) {
  ...
  return (
    str.split('').reduce((acc, char) => {
      ...
      if (char in bracketObj) {
        return [...acc, char];
      }
      if (Object.values(bracketObj).includes(char)) {
        const lastOpen = acc.pop();
        if (!lastOpen || bracketObj[lastOpen] !== char) {
          return false;
        }
        return acc;
      }
...
function toNaryString(num, n) {
  ...
  let basicNum = num;
  while (basicNum > 0) {
    const remainder = basicNum % n;
    result.unshift(remainder);
    basicNum = Math.floor(basicNum / n);
  }
  return result.join('');
}
...
function getCommonDirectoryPath(pathes) {
  const result = pathes.reduce((acc, el) => {
    const accParts = acc.split('/');
    const elParts = el.split('/');
    const arr = [];
    for (let i = 0; i < accParts.length && i < elParts.length; i += 1) {
      if (accParts[i] === elParts[i]) {
        arr.push(accParts[i]);
      } else {
        break;
      }
  ...
  if (result === '') {
    const allSlash = pathes.every((path) => path.startsWith('/'));
    return allSlash ? '/' : '';
  }
  return `${result}/`;
}
